...
class Ankieta extends Controller
{
    ...
    public function ReturnSingleUserAnkieta(int $id) {
        $user_id = Auth::user()->id;
        $ifAdmin = $user_id == 1;
        $answer = DB::table('odpowiedzis')->leftJoin('ankieties', 'ankieties.id', '=', 'odpowiedzis.ankieta_id')->where('odpowiedzis.user_id', '=', $user_id)->where('odpowiedzis.ankieta_id', '=', $id)->get();
    ...
    public function GetEditAnkieta(int $id) {
        $ifAdmin = Auth::user()->id == 1;
        $res =  DB::table('ankieties')->select()->where('id', '=', $id)->get();
        return view('pages.editpage', compact('ifAdmin', 'res'));
    }
    
    public function EditAnkieta(int $id, Request $req) {
        DB::table('ankieties')->where("id", "=", $id)->update([
            "updated_at" => Carbon::now(),
            "nazwa_ankiety" => $req->nazwa_ankiety,
